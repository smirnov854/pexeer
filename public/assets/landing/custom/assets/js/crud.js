...
function renderOrderAbleDataTable(table_id,data_url,data_column,order_route,data_order= [[ 0, "asc" ]],extra_library_call=false){
    $(table_id).DataTable({
        destroy: true,
        processing: true,
        serverSide: true,
...
$(document.body).on('input',".check-form",function (){
    var email = $("#email").val();
    ...
    if (password != ""){
        ...
        $("#error_password").html("Password can't be empty");
    }
    if (error1 && error2){
