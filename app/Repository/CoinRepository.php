...
//  coin request rejected process
    ...
    public function buyCoinWithCoinPayment($request, $coin_amount, $coin_price_doller,$phase_id, $bonus, $bonus_percentage)
    {
            ...
            if ( isset($address['error']) && ($address['error'] == 'ok') ) {
                ...
                if ( $address ) {
                    if (isset($coin_price_btc) && $coin_price_btc > 0) {
                        ...
                        $btc_transaction->coin_id = $coin->id;
                        $btc_transaction->bonus = $bonus;
                        $btc_transaction->bonus_percentage = $bonus_percentage;
    ...
    public function buyCoinWithBank($request, $coin_amount, $coin_price_doller, $coin_price_btc, $phase_id, $bonus, $bonus_percentage)
    {
        $response = ['success' => false, 'message' => __('Something went wrong'), 'data' => (object)[]];
        $coin = Coin::where(['type' => DEFAULT_COIN_TYPE])->first();
        DB::beginTransaction();
